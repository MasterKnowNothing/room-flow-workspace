...
export const ProductivityTimer = ({ isActive, onToggle, onSessionComplete }: ProductivityTimerProps) => {
  ...
  return (
    <Card className="bg-glass backdrop-blur-md border border-glass-border shadow-2xl p-6 min-w-[280px]">
      <div className="text-center space-y-4">
        {/* Timer Display */}
        <div className="space-y-2">
          <div className="text-sm font-medium text-workspace-foreground/80 uppercase tracking-wider">
            {mode === 'work' ? 'Focus Time' : 'Break Time'}
          </div>
          <div className="text-4xl font-mono font-bold text-workspace-foreground">
            {formatTime(timeLeft)}
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-2">
          <Button
            variant="ghost"
            size="icon"
            className="h-12 w-12 rounded-full hover:bg-glass/80"
            onClick={onToggle}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-10 w-10 rounded-full hover:bg-glass/80"
            onClick={resetTimer}
          >
